...
def processing(q):
    ...
    for i in range(0, num, chunk_size):
        ...
        mask = MaskData[i:i+chunk_size,]
        print(data.shape, mask.shape)

        intensity_num += np.sum(data, axis=0)

        bitwise_bad = np.bitwise_and(mask, MASK_BAD)
        bitwise_good = np.bitwise_and(mask, MASK_GOOD)

        condition_good = bitwise_good != mask
        condition_bad = bitwise_bad
        

        join_condition = np.logical_or(condition_good, condition_bad)
        tmp_int = np.where(join_condition == True, 0, data)
        tmp_click_count = np.where(join_condition == True, 0, 1)
        
        intensity += np.sum(tmp_int, axis=0) #per-pixel intensity
        click_count += np.sum(tmp_click_count, axis=0)
